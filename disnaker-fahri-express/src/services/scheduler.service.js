...
class SchedulerService {
  ...
  init() {
    ...
    // Absensi reminder - check every minute for dynamic reminder times from settings
    ...
    console.log('⏰ Absensi reminder: Every minute (checks reminder time settings)');
  }

  /**
   * Check if it's time to send absensi reminder (at reminder time settings)
   */
  async checkAbsensiReminder() {
    ...
    const jamMasuk = settings.reminder_masuk_time || settings.jam_masuk || '08:00';
    const jamPulang = settings.reminder_pulang_time || settings.jam_pulang || '16:00';
