...
// Default koordinat kantor DPMD Bogor. Nilai aktif bisa diubah dari absensi_settings.
...
async function getAbsensiSettings() {
  ...
  return {
    ...
    kantorLatitude: parseNumberSetting(map.kantor_latitude, DEFAULT_KANTOR_LAT),
    kantorLongitude: parseNumberSetting(map.kantor_longitude, DEFAULT_KANTOR_LNG),
    maxDistanceMeters: parseNumberSetting(map.max_distance_meters, DEFAULT_MAX_DISTANCE_METERS),
    reminderMasukTime: map.reminder_masuk_time || map.jam_masuk || '08:00',
    reminderPulangTime: map.reminder_pulang_time || map.jam_pulang || '16:00',
  };
}
...
const absensiController = {
  ...
  async clockIn(req, res) {
    try {
      ...
      // Hitung jarak dari kantor sesuai pengaturan aktif
      const jarak = calculateDistance(
        parseFloat(latitude),
        parseFloat(longitude),
        absensiSettings.kantorLatitude,
        absensiSettings.kantorLongitude
      );

      // Cek jarak hanya untuk mode 'hadir' (WFH/WFA/dinas luar bebas lokasi)
      if (absensiMode === 'hadir' && jarak > absensiSettings.maxDistanceMeters) {
        return res.status(400).json({
          success: false,
          message: `Anda berada ${Math.round(jarak)} meter dari kantor. Maksimal jarak absensi adalah ${Math.round(absensiSettings.maxDistanceMeters)} meter.`
        });
      }
  ...
  async clockOut(req, res) {
    try {
      ...
      if (existing.status === 'hadir' && jarak > absensiSettings.maxDistanceMeters) {
        return res.status(400).json({
          success: false,
          message: `Anda berada ${Math.round(jarak)} meter dari kantor. Maksimal jarak absensi adalah ${Math.round(absensiSettings.maxDistanceMeters)} meter.`
        });
  ...
  async getSettings(req, res) {
    try {
      ...
      if (!result.kantor_latitude) result.kantor_latitude = String(DEFAULT_KANTOR_LAT);
      if (!result.kantor_longitude) result.kantor_longitude = String(DEFAULT_KANTOR_LNG);
      if (!result.max_distance_meters) result.max_distance_meters = String(DEFAULT_MAX_DISTANCE_METERS);
      if (!result.reminder_masuk_time) result.reminder_masuk_time = result.jam_masuk || '08:00';
      if (!result.reminder_pulang_time) result.reminder_pulang_time = result.jam_pulang || '16:00';
   ...
   * Body: { jam_masuk, jam_pulang, toleransi_terlambat, jam_buka_absen, jam_tutup_absen, kantor_latitude, kantor_longitude, max_distance_meters, reminder_masuk_time, reminder_pulang_time }
   */
  async updateSettings(req, res) {
    try {
      const {
        ...
        max_distance_meters,
        reminder_masuk_time,
        reminder_pulang_time
      } = req.body;
      const userId = BigInt(req.user.id);

      const isTime = (value) => typeof value === 'string' && /^\d{2}:\d{2}$/.test(value);
      const isValidNumber = (value) => value !== null && value !== undefined && String(value).trim() !== '' && Number.isFinite(Number(value));

      const timeFields = [
        ['jam_masuk', jam_masuk],
        ['jam_pulang', jam_pulang],
        ['jam_buka_absen', jam_buka_absen],
        ['jam_tutup_absen', jam_tutup_absen],
        ['reminder_masuk_time', reminder_masuk_time],
        ['reminder_pulang_time', reminder_pulang_time],
      ];

      for (const [key, value] of timeFields) {
        if (value !== undefined && !isTime(value)) {
          return res.status(400).json({ success: false, message: `${key} harus format HH:mm` });
        }
      }

      if (kantor_latitude !== undefined && (!isValidNumber(kantor_latitude) || Number(kantor_latitude) < -90 || Number(kantor_latitude) > 90)) {
        return res.status(400).json({ success: false, message: 'Latitude kantor harus angka antara -90 sampai 90' });
      }

      if (kantor_longitude !== undefined && (!isValidNumber(kantor_longitude) || Number(kantor_longitude) < -180 || Number(kantor_longitude) > 180)) {
        return res.status(400).json({ success: false, message: 'Longitude kantor harus angka antara -180 sampai 180' });
      }

      if (max_distance_meters !== undefined && (!isValidNumber(max_distance_meters) || Number(max_distance_meters) <= 0)) {
        return res.status(400).json({ success: false, message: 'Radius maksimal absensi harus lebih dari 0 meter' });
      }
      ...
      if (kantor_latitude !== undefined) settingsToUpdate.push({ key: 'kantor_latitude', value: String(kantor_latitude), description: 'Latitude lokasi kantor untuk validasi absensi' });
      if (kantor_longitude !== undefined) settingsToUpdate.push({ key: 'kantor_longitude', value: String(kantor_longitude), description: 'Longitude lokasi kantor untuk validasi absensi' });
      if (max_distance_meters !== undefined) settingsToUpdate.push({ key: 'max_distance_meters', value: String(max_distance_meters), description: 'Radius maksimal absensi dari lokasi kantor (meter)' });
      if (reminder_masuk_time !== undefined) settingsToUpdate.push({ key: 'reminder_masuk_time', value: reminder_masuk_time, description: 'Jam kirim reminder absen masuk' });
      if (reminder_pulang_time !== undefined) settingsToUpdate.push({ key: 'reminder_pulang_time', value: reminder_pulang_time, description: 'Jam kirim reminder absen pulang' });
  ...
  async getRekapPegawai(req, res) {
    try {
      ...
      const rawRecords = await prisma.absensi_pegawai.findMany({
        where,
        select: { id: true, user_id: true, status: true, tanggal: true, jam_masuk: true, jam_keluar: true }
      });
      ...
      const pegawaiRekap = allUsers.map(u => {
        ...
        return {
          ...
          records: userRecords.map(r => ({
            ...
            telat_masuk_menit: r.telat_masuk_menit || 0,
          })),
        };
      });
